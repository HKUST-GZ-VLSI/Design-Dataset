        ...
        if file.endswith("json"):
            ...
            for node, attr in hier_graph.nodes(data=True):
                ...
                if 'net' != attr['inst_type'] and 'values' in attr:
                    ...
                    feature.append(0)
                if 'switched_capacitor_filter' in file:
                    if 'xi0|telescopic_ota' in node.lower():
                        feature.append(1)
                    else:
                        feature.append(0)
                # elif 'mimo' in file:
                #     if 'ximixer' in node.lower():
                #         feature.append(2)
                #     else:
                #         feature.append(0)
                else:
                    if '|fully_differential_' in node.lower() or 'ota' in node.lower() or 'beamformer' in node.lower():
                        #OTA
                        feature.append(4)
                    elif 'oscillator' in node.lower() or 'vlo' in node.lower() or 'BPF' in node.upper() or 'ilo' in node.lower():
                        #oscillator
                        feature.append(3)
                        ...
                        #lna
                        feature.append(1)
                    elif [True for n in ['clk','vssd','vss','vdd', 'gnd', 'vrf', 'antenna', 'bias', 'digital', 'vtune', 'gs1', 'gs2', 'control', 'vout'] if n in node.lower()]:
                        #mixer
                        feature.append(0)
                    else:
...
if __name__ == '__main__':
    ...
    for data_type in ['switched_capacitor_filter', 'MIMO', 'wideband_mixer_RX', 'phased_array_netlist']:
